...
En pocas palabras, a través de un ataque de delegación restringida basada en recursos podemos agregar una computadora bajo . con la capacidad de hacerse pasar por un usuario con altos privilegiado en el dominio, como El atributo ms-ds-machineaccountquota debe ser superior a 0. Este atributo controla el WriteDACL ) a través de una computadora unida al dominio (en este caso, el controlador de dominio). el administrador ( GenéricoTodo entradas para $FAKE-COMP01 como este usuario privilegiado, dándonos control sobre todo el dominio. El ataque se basa en tres requisitos previos: , nuestro control al dominio; Llamemos a esta computadora $FAKE-COMP01 para permitir que $FAKE-COMP01 actúe en su nombre. Luego, actuando en nombre del DC podemos solicitar Kerberos , De forma predeterminada, cualquier miembro de este grupo puede agregar hasta 10 computadoras al dominio. y configurar el controlador de dominio (DC) Necesitamos un shell o ejecución de código como usuario de dominio que pertenece al grupo de Usuarios autenticados . Ataque de delegación (RBCD) y escalada de nuestros privilegios. , cantidad de computadoras que los usuarios de dominio autenticados pueden agregar al dominio. Nuestro usuario actual o un grupo del que nuestro usuario es miembro, debe tener privilegios de ESCRITURA BloodHound menciona que debido al privilegio GenericAll podemos realizar una función restringida basada en recursos. Una vez generados los tickets de Kerberos, podemos pasar el ticket (PtT) y autenticarnos.